  ...
  <v-card>
    <v-toolbar color="amber darken-3" dark flat dense>
      <v-toolbar-title class="subheading"
        >Split dataset into train and test</v-toolbar-title
      >
    </v-toolbar>
    <v-card-text>
      <v-form>
        <v-layout row wrap>
          ...
          <v-flex xs12 v-if="dataset">
            <v-subheader class="pa-0"
              >Split ratio ({{ trainPercent | formatPercent }} vs
              {{ testPercent | formatPercent }})</v-subheader
            >
            <v-text-field
              v-model="train"
              readonly
          ...
          <v-flex xs12 v-if="dataset">
            ...
            <v-text-field
              v-model="trainDatasetName"
              label="Train dataset name"
            ></v-text-field>
            <v-text-field
              v-model="testDatasetName"
              label="Test dataset name"
            ></v-text-field>
          </v-flex>
          <v-flex xs12 v-if="dataset">
            <v-alert :value="done" type="success" transition="scale-transition">
              Datasets <strong>{{ trainDatasetName }}</strong> and
              <strong>{{ testDatasetName }}</strong> has been successfully
              created.
            </v-alert>
            <v-alert
              :value="submitError"
  ...
  methods: {
    async submit() {
      try {
        ...
        await this.$http.put(
          `/api/v1/preprocessing/dataset/${this.dataset}/split`,
          {
